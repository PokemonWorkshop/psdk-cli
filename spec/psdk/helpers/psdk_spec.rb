...
RSpec.describe Psdk::Cli::PSDK do
  describe '.unuse_local_pokemonsdk' do
    ...
    context 'when psdk folder exists' do
      before do
      ...
      context 'when it is not a git submodule' do
        context 'with delete: true' do
          let(:delete_option) { true }

          it 'deletes the folder' do
            expect(FileUtils).to receive(:rm_rf).with(psdk_path)
            expect(Psdk::Cli::PSDK).to receive(:puts).with("Successfully set project to use Pokémon Studio's PSDK version")
        ...
        context 'with delete: false' do
          ...
          context 'when _old folder already exists' do
            ...
            it 'exits with error' do
              expect(Psdk::Cli::PSDK).to receive(:puts).with("[Error] Folder `#{new_path}` already exists. Please remove it manually.")
      ...
      context 'when it IS a git submodule' do
        before do
        ...
        context 'with delete: false' do
          ...
          it 'exits with error advising to remove submodule manually' do
            expect(Psdk::Cli::PSDK).to receive(:puts).with("[Error] Cannot use Studio's PSDK version if the project has a submodule.")
        ...
        context 'with delete: true' do
          ...
          it 'fails to deinit submodule' do
            ...
            expect(Psdk::Cli::PSDK).to receive(:puts).with('[Error] Failed to remove the submodule (Failed to deinit pokemonsdk submodule)')
            expect { Psdk::Cli::PSDK.unuse_local_pokemonsdk(delete: delete_option) }.to raise_error('Exited 1')
          end
          ...
          it 'fails to remove submodule' do
            ...
            expect(Psdk::Cli::PSDK).to receive(:puts).with('[Error] Failed to remove the submodule (Failed to remove pokemonsdk submodule)')
            expect { Psdk::Cli::PSDK.unuse_local_pokemonsdk(delete: delete_option) }.to raise_error('Exited 1')
          end
        end
      end
